...
# NOTE: change this to your region!!
lat.lb <- 38
lat.ub <- 43
lon.lb <- -107
lon.ub <- -100
...
for (file in nc.files){
  ...
  print(paste("at file: ", count, "     out of: ", length(nc.files), sep = ""))
  count <- count + 1
  ...
  if (any(to.keep)){ 
    ...
    rm(other.vars.temp, region.cut)
    
  } # end  if (any(to.keep))
  
  # Clear .nc file from memory
  nc_close(this.nc)
  rm(this.nc, this.gas, this.lat, this.lon, this.time, to.keep)
  
  # Check for unique months in the data vectors
  month <- month(as_datetime(time))
  unique.months <- unique(month)
  
  # If there is more than one month present, save the completed month to an .RData file
  if (length(unique.months) > 1){
    
    other.vars.to.save <- vector(mode = "list", length = num.vars)
    
    to.save <- month(as_datetime(time)) == unique.months[1]
    
    lat.to.save  <- lat[to.save]
    lon.to.save  <- lon[to.save]
    gas.to.save  <- gas[to.save]
    time.to.save <- time[to.save]
    
    for(i in 1:num.vars){
      other.vars.to.save[[i]] <- other.vars[[i]][to.save]
    }
    
    # Put all of the vectors into a data frame
    data <- data.frame(lat.to.save, lon.to.save, time.to.save, gas.to.save)
    colnames(data) <- c("lat", "lon", "time", "gas")
    num.perm.cols <- ncol(data)
    
    for (i in 1:num.vars){
      data <- cbind(data, other.vars.to.save[[i]])
    }
    ...
    # Save to .RData file
    saveRDS(data, file = paste( "NE_CO_",
                                product,
                                "_", 
                                year(as_datetime(time[1])), 
                                "_", 
                                unique.months[1],
                                ".RData",
                                sep = "" ) )
    
    # Remove the data that we just saved to an .RData file, leave the rest
    lat <- lat[!to.save]
    lon <- lon[!to.save]
    gas <- gas[!to.save]
    time <- time[!to.save]
    ...
    for(i in 1:num.vars){
      other.vars[[i]] <- other.vars[[i]][!to.save]
    }
    
    # Clean up workspace
    rm(to.save, lat.to.save, lon.to.save, gas.to.save, time.to.save, 
       unique.months, month, data, other.vars.to.save)
    gc()
    
  } # end if (unique.months > 1)
  
} # end for (file in nc.files)


# Now save leftover data into the last month
...
# Save to .RData file
saveRDS(data, file = paste( product,
                            "_", 
                            year(as_datetime(time[1])),
                            "_", 
                            unique.months[1], 
